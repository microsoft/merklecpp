...
namespace merkle
{
    ...
    void hash(Node* n, size_t indent = 2) const
    {
      ...
      while (!hashing_stack.empty())
      {
        ...
        else
        {
          ...
          if(n->left->hash <  n->right->hash){
            HASH_FUNCTION(n->left->hash, n->right->hash, n->hash);
          }else{
            HASH_FUNCTION(n->right->hash, n->left->hash, n->hash);
          }
          statistics.num_hash++;
          MERKLECPP_TRACE(
  ...
  static inline void keccak256_crypto(
      const HashT<32>& l,
      const HashT<32>& r,
      HashT<32>& out)
  {
      ...
      uint8_t block[32 * 2];
      memcpy(&block[0], l.bytes, 32);
      memcpy(&block[32], r.bytes, 32); // OMGGGGGGG
  ...
  /// @brief  path with default hash size and crypto hash function
  typedef PathT<32, keccak256_crypto> KeccakPath;

  /// @brief  tree with default hash size and crypto hash function
  typedef TreeT<32, keccak256_crypto> KeccakTree;
};
