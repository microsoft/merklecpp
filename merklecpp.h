...
namespace merkle
{
  ...
  struct HashT
  {
    ...
    void deserialse(const std::vector<uint8_t>& buffer, size_t& position)
    {
  ...
  public:
    ...
    void deserialse(const std::vector<uint8_t>& bytes, size_t& position)
    {
      ...
      elements.clear();
      _leaf.deserialse(bytes, position);
  ...
  public:
    ...
    /// @param bytes Bytes containing a serialised tree
    ...
    /// @brief Deserialises a tree
    /// @param bytes Bytes containing a serialised tree
    /// @param position Position of the first byte within `bytes`
    ...
    /// @note This invalidates all indicies smaller than `index` and
    ...
    /// @brief Retracts a tree up to some leaf
    /// @param index Leaf index to retract the tree to
    /// @note This invalidates all indicies greater than `index` and
    /// no paths from them to the root can be extracted anymore.
    ...
    /// `index` was the last, right-most leaf index in the tree. It is
    /// equivalent to retracting the tree to `index` and then extracting the
    /// root.
    ...
    /// @note This extracts a path at a past state, when `index` was the last,
    /// right-most leaf index in the tree. It is equivalent to retracting the
    /// tree to `index` and then extracting the path of `index`.
    ...
    /// @note The smallest leaf index for which it is safe to extract roots and paths.
    ...
    /// @note The greatest leaf index for which it is safe to extract roots and paths.
    ...
    /// @brief Computes the minumal number of bytes required to serialise the tree
